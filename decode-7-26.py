...
class BatchDecodeWithPagedKVCacheWrapper:
    ...
    def __init__(
        ...
        if use_tensor_cores:
            ...
            if use_cuda_graph:
                ...
                self._qo_indptr_buf_cpu = torch.arange(
                    self._fixed_batch_size + 1,
                    dtype=torch.int32,
                    device='cpu'
                )
    ...
    def begin_forward(
        ...
        indptr_cpu: Optional[torch.Tensor] = None,
        last_page_len_cpu: Optional[torch.Tensor] = None,
    ):
        ...
        if self.use_tensor_cores:
            ...
            self._wrapper.begin_forward(
                ...
                self._qo_indptr_buf_cpu,
                # self._qo_indptr_buf if indptr_cpu is None else indptr_cpu.clone(),
                # self._qo_indptr_buf if indptr_cpu is None else indptr_cpu.clone(),
                # indptr,
                indptr if indptr_cpu is None else indptr_cpu,
                batch_size,
                num_qo_heads,
                num_kv_heads,
                head_dim,
            ...
            self._wrapper.begin_forward(
                self._workspace_buffer,
                indptr if indptr_cpu is None else indptr_cpu,
                last_page_len if last_page_len_cpu is None else last_page_len_cpu,
                # indptr,
                # last_page_len,
                batch_size,
                num_qo_heads,
