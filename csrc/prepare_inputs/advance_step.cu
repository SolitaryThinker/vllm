...
__global__ void advance_step_flashinfer_kernel(
                                    ...
                                    int const* block_tables_ptr,
                                    int64_t const block_tables_stride,
                                    int* paged_kv_last_page_len_ptr,
...
__global__ void advance_step_flashinfer_indptr_kernel(
  int num_threads, int num_seqs, int num_queries,
                                    int* paged_kv_indptr_ptr,
                                    int* block_table_bound_ptr) {
...
__global__ void advance_step_flashinfer_indices_kernel(
  ...
  if (row < num_queries && col < block_table_bound_ptr[row]) {
      ...
      // printf("block_tables_ptr[row * block_tables_stride + col]: %d\n", block_tables_ptr[row * block_tables_stride + col]);
      // printf("paged_kv_indptr_ptr[row]: [%d + %d = %d] = %d\n", paged_kv_indptr_ptr[row], col, paged_kv_indptr_ptr[row] + col, 
        // block_tables_ptr[row * block_tables_stride + col]);
...
void advance_step_flashinfer(int num_seqs, int num_queries, int block_size,
                  torch::Tensor& input_tokens,      // type: long
                  torch::Tensor& sampled_token_ids, // type: long
                  torch::Tensor& input_positions,   // type: long
                  torch::Tensor& seq_lens,          // type: int
  ...
  if ((blocks * threads) / block_tables.stride(0) < num_queries) {
    TORCH_CHECK(false, "multi-step: not enough threads to map block_table to"
                 "FlashInfer's paged_kv_indices on GPU. Try reducing the number of seqs,",
                " increasing the block size or take smaller steps.", " num_queries = ", num_queries,
                " block_tables.stride(0) = ", block_tables.stride(0), " blocks = ", blocks,
                " max_threads = ", threads);
  }
...
void advance_step_flashinfer(int64_t num_seqs, int64_t num_queries, int64_t block_size,
                  torch::Tensor& input_tokens, torch::Tensor& sampled_token_ids,
                  torch::Tensor& input_positions, torch::Tensor& seq_lens,
                  torch::Tensor& slot_mapping, torch::Tensor& block_tables,
                  ...
                  torch::Tensor& block_table_bound) {
  prepare_inputs::advance_step_flashinfer(num_seqs, num_queries, block_size, input_tokens,
                               sampled_token_ids, input_positions, seq_lens,
                               slot_mapping, block_tables, paged_kv_indices,
